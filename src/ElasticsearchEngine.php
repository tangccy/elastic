...
class ElasticsearchEngine extends Engine
{
    ...
    protected function performSearch(Builder $builder, array $options = [])
    {
         ...
         * 指定字段query
         */
        if (method_exists($builder->model, 'queryDefaultFields')) {
            $params['body']['query']['bool']['must'][0]['query_string']['fields'] = $builder->model->queryDefaultFields();
        }
    ...
    public function map($results, $model)
    {
        ...
        return collect($results['hits']['hits'])->map(function ($hit) use ($model, $models) {
            ...
            if (isset($hit['highlight'])) {
                $one->setAttribute('highlight',$hit['highlight']);
            }
            return $one;
        });
    }
     ...
     * 获取模型索引
     *
     * @author  lanzhanqiao (zhanqiaolan985@gmail.com)
     * @data  2018/6/28
     * @param Model $model
     * @return mixed
     */
    private function getIndex(Model $model)
    {
        if (method_exists($model, 'esIndex')) {
            return  $model->esIndex();
        }
